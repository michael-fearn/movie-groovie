...
module.exports = {
  addMovie: (req, res) => {
    ...
    if (flag) {
      ...
      res.status(404).send("Bad request or duplicate");
    }
  },
  ...
  discoverMovies: (req, res) => {
    axios
      .get(
        `https://api.themoviedb.org/3/discover/movie?year=${+req.query
          .year}&api_key=${API_KEY}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1`
      )
      .then(response => res.send(response.data.results).status(200));
  },
  ...
  //         .then(response => {
  //           res.send(response.data[0].content)})         
  // }
};
